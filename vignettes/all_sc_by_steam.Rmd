...
Load SC se object:
```{r}
# se <- readRDS("data/single_cell/Seurat_objects/mock_data_single_nuclei_human_brain.rds")
# se
...
```{r}
# clustered_metacells <- generate_metacell_clusters(se, cluster_anno = "supercluster_term", verbose = FALSE)
```
...
Save cluster annotations
```{r, eval=T}
# seurat_clusters <- as.data.frame(meta_cell_seurat_all_sum_with_metadata$supercluster_term)
# seurat_clusters$Barcode <- colnames(meta_cell_seurat_all_sum_with_metadata)
# seurat_clusters$seurat_clusters <- seurat_clusters$`meta_cell_seurat_all_sum_with_metadata$supercluster_term`
# seurat_clusters$`meta_cell_seurat_all_sum_with_metadata$supercluster_term` <- NULL
# write.table(seurat_clusters,file="data/Seurat_objects/seurat_clusters.csv", col.names = T, row.names = F,quote = FALSE,sep = ";")

#seurat_clusters <- read.table("data/single_cell/Seurat_objects/seurat_clusters.csv",
#                               header = TRUE,
#                               sep = ";",
...
se <- readRDS("data/single_cell/Seurat_objects/se_Metacell_normalized.RDS")
...
OpenTargets_SCZ_Genetic_ <- as.character(t(read.table(file = "data/single_cell/OpenTargets_gene_sets/Human_OpenTargets_SCZ_Genetic.csv")))
length(OpenTargets_SCZ_Genetic_)

OpenTargets_SCZ_Drugs_ <- as.character(t(read.table(file = "data/single_cell/OpenTargets_gene_sets/Human_OpenTargets_SCZ_Drugs.csv")))
...
```{r}
window50_rank_list_SCZ_Genetic <- readRDS(file = "data/single_cell/OpenTargets_windowLists/Human_window50_rank_list_SCZ_Genetic.RDS")
window50_rank_list_SCZ_Drugs <- readRDS(file = "data/single_cell/OpenTargets_windowLists/Human_window50_rank_list_SCZ_Drugs.RDS")
```
...
```{r}
perm_files <- c(
  "data/single_cell/Permutation_matrices/perm.mat_p10000_g50.RDS",
  "data/single_cell/Permutation_matrices/perm.mat_p10000_g1315.RDS",
  "data/single_cell/Permutation_matrices/perm.mat_p10000_g218.RDS"
)
...
```{r}
DEG_genes <- read.csv("data/single_cell/DEG_output/DE_genes_human_snRNAseq_Metacells.csv", 
                     stringsAsFactors = FALSE, 
                     sep = ";",
                     header = TRUE)
